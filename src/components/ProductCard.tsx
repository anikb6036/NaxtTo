  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square w-full overflow-hidden bg-[#f5f5f7]">
        ...
        {/* Quick View & Instant Add Overlay Buttons */}
        <div className="absolute bottom-3 inset-x-3 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 font-sans">
          <button
            ...
            className="flex-1 py-2.5 bg-white/95 hover:bg-white text-[#1d1d1f] text-[9px] font-bold tracking-[0.2em] uppercase rounded-xs backdrop-blur-xs flex items-center justify-center gap-1.5 shadow-sm transition-all"
          >
          ...
          <button
            ...
            className={`p-2.5 rounded-xs transition-all flex items-center justify-center shadow-sm ${
              addedAnimation
          ...
          >
            {addedAnimation ? <Check className="w-3.5 h-3.5" /> : <ShoppingBag className="w-3.5 h-3.5 text-white" />}
          </button>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1 justify-between space-y-3">
        <div>
          {/* Metal & Rating Line */}
          <div className="flex items-center justify-between text-[9px] text-[#86868b] uppercase tracking-[0.2em] font-sans mb-1">
            <span className="truncate max-w-[170px]">
              {product.metalName}
            </span>
            <div className="flex items-center gap-1 text-[#1d1d1f]">
              <Star className="w-2.5 h-2.5 fill-[#c5a059] text-[#c5a059]" />
              <span className="font-bold text-[10px]">{product.rating.toFixed(1)}</span>
            </div>
          </div>

          {/* Product Name */}
          <h3 className="font-serif text-lg font-light text-[#1d1d1f] leading-snug group-hover:opacity-60 transition-opacity">
            {product.name}
          </h3>

          <p className="text-xs text-[#86868b] font-serif italic line-clamp-1 mt-0.5">
            {product.subtitle}
          </p>
        </div>
          ...
          <div className="text-right">
            <div className="flex items-baseline gap-1.5 justify-end">
              {product.originalPrice && (
                <span className="text-[10px] text-[#86868b] line-through font-sans">
                  {currencySymbol}{product.originalPrice}
                </span>
              )}
              <span className="font-serif text-base font-light text-[#1d1d1f]">
