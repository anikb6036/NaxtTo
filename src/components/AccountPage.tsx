...
import { subscribeToUserOrders, subscribeToSingleOrder, updateOrderStatusInFirestore, saveOrderToFirestore, getDeletedOrderIds, DUMMY_ORDER_IDENTIFIERS } from '../utils/userStorage';
...
export const AccountPage: React.FC<AccountPageProps> = ({
  ...
  const displayOrders = React.useMemo(() => {
    ...
    const isOrderValid = (o: Order | null | undefined): o is Order => {
      if (!o || !o.id) return false;
      const orderNum = o.orderNumber || o.id;
      if (deletedIds.has(o.id) || deletedIds.has(orderNum)) return false;
      if (DUMMY_ORDER_IDENTIFIERS.has(o.id) || DUMMY_ORDER_IDENTIFIERS.has(orderNum)) return false;
      return true;
    };

    // Initial from user profile
    (user.orderHistory || []).forEach(o => {
      if (isOrderValid(o)) map.set(o.id, o);
    });
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6 animate-fadeIn">
                ...
                {displayOrders.some(o => o.status !== 'Delivered' && o.status !== 'Cancelled') ? (
                  ...
                  <div className="p-5 bg-[#f5f5f7] rounded-2xl border border-[#e5e5ea] flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center text-[#86868b] shrink-0 shadow-xs border border-[#e5e5ea]">
                      <Package className="w-4.5 h-4.5" />
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold text-[#1d1d1f]">No Recent Orders</h4>
                      <p className="text-xs text-[#6e6e73]">When you purchase from our fine jewellery collection, your order details and real-time consignment tracking will appear here.</p>
                    </div>
                  </div>
                )}
              </div>
